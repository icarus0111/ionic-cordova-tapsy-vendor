...
export class MyBookingComponent implements OnInit {
  ...
  this.maxRecord = 10; //--- Maximum value show each pagination
  ...
  getUpcomingJobList() {
    ...
    this.apiService.getLimitJobsListByUser(payload).subscribe(async (data: any) => {
      ...
      this.showLoader = false;
      const alert = await this.alert.presentAlertConfirm('Alert!', 'Sorry, no data found.');
      alert.present();
  ...
  getCompletedJobList() {
    ...
    this.apiService.getLimitJobsListByUser(payload).subscribe(async (data: any) => {
      if(data && data.TAP_RES) {
        let decrypted = this.helper.decryptResponceData(data.TAP_RES);
        console.log('this.completedJobs: ', decrypted);
        if(decrypted.status) {
          if(decrypted.data !== null && decrypted.data.length > 0) {
            ...
            //--- Check wheather no of product return is less than maximum record
            //console.log('this.completedJobs.length', this.completedJobs.length);
  ...
  // Not require from 15-01-2020 as scrolling time dynamic data loaded
  getJobList() {
    this.showLoader = true;
    ...
    this.apiService.getAllJobsListByVendor(payload).subscribe(async (data: any) => {
      if(data && data.TAP_RES) {
        let decrypted = this.helper.decryptResponceData(data.TAP_RES);
